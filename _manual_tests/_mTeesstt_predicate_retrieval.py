import os

from nl_2_fol.prompting.retrieve_relevant_predicates import (
    SemanticPredicateRetriever,
)

with open(os.path.join("tests", "_manual_tests", "predicates_list.txt"), "r") as f:
    predicates = [line.strip() for line in f.readlines()]


retriever = SemanticPredicateRetriever()
retriever.add_predicates(predicates)


definition = """
A carbon oxoacid acid [sic] carrying at least one ‒C(=O)OH group and having the
 structure RC(=O)OH, where R is any any monovalent functional group.
 Carboxylic acids are the most common type of organic acid.
"""

matches = retriever.retrieve_relevant_predicates(definition, top_k=None, threshold=0.35)
print(matches)
